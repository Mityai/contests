...
vector< pair<ll, vector<int> > > ansEdges;
ll recoverFlow(int v, ll cur_flow) {
    if (v == t) {
        return cur_flow;
    }
    ...
    for (int e : E[v]) {
        if (flow[e] > 0) {
            ansEdges.back().se.push_back(e / 2);
            cur_flow = min(cur_flow, flow[e]);
            int del_flow = recoverFlow(to[e], cur_flow);
            flow[e] -= del_flow;
            return del_flow;
        }
    }
...
int main() {
    ...
    while (ansFlow > 0) {
        ansEdges.push_back({0, vector<int>()});
        ansEdges.back().fi = recoverFlow(s, INF);
        ansFlow -= ansEdges.back().fi;
    }

    printf("%d\n", (int)ansEdges.size());
    for (auto path : ansEdges) {
        printf("%lld %d ", path.fi, (int)path.se.size());
        for (int id : path.se) {
            printf("%d ", id + 1);
        }
        puts("");
