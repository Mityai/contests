...
int mem[2][maxN], c[maxN];
int sn, n, m, *a = mem[0], *b = mem[1];

inline bool aless( int i, int j ) { return a[i] < a[j]; }
...
struct Seg
{
  ...
  inline int get( int x )
  {
    if (set != -1)
      return set >= x ? len : 0;
    a[n] = x;
    return (b + st + len) - lower_bound(b + st, b + st + len, n, aless);
  }
};
...
int Split( int x )
{
  ...
  if (pos != x)
  {
    ...
    int k = x - pos, L = s[i].st, R = L + s[i].len - 1;
    s[i + 1].set = s[i].set;
    s[i + 1].len = s[i].len - k;
    s[i].len = k;
    if (s[i].rev)
    ...
    int L1 = s[i].st, R1 = s[i].st + s[i].len - 1;
    int k1 = s[i].st, k2 = s[i + 1].st;
    cnt2 += R - L;
    forab(j, L, R)
      if (L1 <= b[j] && b[j] <= R1)
        c[k1++] = b[j];
